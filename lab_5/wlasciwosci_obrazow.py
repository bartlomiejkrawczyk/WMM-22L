...
"""
image_vdiff = cv2.addWeighted(
    image[1:, :], 1, image[:-1, :], -1, 0, dtype=cv2.CV_16S)
...
# połączenie tablic w kierunku pionowym, czyli 'wiersz za wierszem'
image_vdiff = np.vstack((image_vdiff_0, image_vdiff))
printi(image_vdiff, "image_vdiff")
cv_imshow(image_vdiff, "image_vdiff")

if not skip_wnd:
    # oczekiwanie na reakcję użytkownika - naciśnięcie klawisza lub zamknięcie okien
    cv2.waitKey(0)
...
# albo: cv2.COLOR_BGR2YUV
image_YCrCb = cv2.cvtColor(image_col, cv2.COLOR_BGR2YCrCb)
printi(image_YCrCb, "image_YCrCb")

hist_Y = cv2.calcHist([image_YCrCb[:, :, 0]], [0],
                      None, [256], [0, 256]).flatten()
hist_Cr = cv2.calcHist([image_YCrCb[:, :, 1]], [0],
                       None, [256], [0, 256]).flatten()
hist_Cb = cv2.calcHist([image_YCrCb[:, :, 2]], [0],
                       None, [256], [0, 256]).flatten()
...
image = cv2.imread("./images/latarnia2_col.png", cv2.IMREAD_UNCHANGED)
